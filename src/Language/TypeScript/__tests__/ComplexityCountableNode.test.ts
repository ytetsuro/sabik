...
describe('ComplexityCountableNode', () => {
    ...
    describe('.isNestLevelUp()', () => {
        it.each([
            ...
            ['function', (<ts.ReturnStatement>map.get('function')!.statements[0]).expression!],
    ...
    describe('.isIncrement()', () => {
        it.each([
            ...
            ['catch', (<ts.TryStatement>map.get('try')!.statements[0]).catchClause!],
            ['conditional', (<ts.ReturnStatement>map.get('conditional')!.statements[0]).expression!],
            ['ampersand', (<ts.BinaryExpression>(<ts.ReturnStatement>map.get('ampersand')!.statements[0]).expression!).operatorToken],
            ['barbar', (<ts.BinaryExpression>(<ts.ReturnStatement>map.get('barbar')!.statements[0]).expression!).operatorToken],
