...
export class ASTNode implements ASTNodeInterface {
    ...
    isMethod() {
      return this.node.kind === ts.SyntaxKind.MethodDeclaration || this.node.kind === ts.SyntaxKind.Constructor
    }
    ...
    getName() {
      ...
      if (this.isClass()) {
        ...
        const methodName = this.node.kind === ts.SyntaxKind.Constructor ? 'constructor' : (<ts.MethodDeclaration> this.node).name.getText()
    ...
    isFauxClass() {
      ...
      return (new ASTNode((<ts.FunctionExpression>this.node).body, this.sourceFile)).getChilds()
    ...
    private getPropertyName(node: ts.BinaryExpression) {
      return node.getText().split('=', 2)[0].replace(/[\n|\r|\s|\t]+/, '')
    }
    ...
    private findPropertyAssignName(node: ts.PropertyAssignment): string[] {
      ...
      switch(parentNode.kind) {
        ...
        case ts.SyntaxKind.BinaryExpression:
          return result.concat(this.getPropertyName(<ts.BinaryExpression>parentNode).split('.').reverse());
        case ts.SyntaxKind.VariableDeclaration:
