...
export class ComplexityCountableNode implements ComplexityCountableNodeInterface {
    ...
    isIncrement() {
      ...
      const allIncrementSyntaxKinds = ComplexityCountableNode
      .incrementSyntaxKinds
      .concat(...ComplexityCountableNode.nestingIncrementSyntaxKinds)
    ...
    private isElse() {
      ...
      return parent.kind === ts.SyntaxKind.IfStatement &&
            (<ts.IfStatement>parent).elseStatement === this.pureNode
    }

    getChilds() {
      return this.node.getChilds().map(node => new ComplexityCountableNode(node))
    }
}
