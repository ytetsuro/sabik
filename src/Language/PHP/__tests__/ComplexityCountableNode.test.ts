...
describe('ComplexityCountableNode', () => {
  ...
  describe('.isNestLevelUp()', () => {
    ...
    it('should returns false when elseif or else statement.', () => {
      const elseIfActual = new ComplexityCountableNode(
        map.get('if')!.getChildren()[0].getChildren()[2]
      );
      const elseActual = new ComplexityCountableNode(
  ...
  describe('.isNestingIncrement()', () => {
    it.each([
      ...
      [
        'conditional',
        map.get('conditional')!.getChildren()[0].getChildren()[0],
      ],
