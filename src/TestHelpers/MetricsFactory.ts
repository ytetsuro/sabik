...
export class MetricsFactory {
  ...
  create(...DSLList: string[]): Metrics[] {
    return DSLList.map(
      (DSL) => new Metrics(this.createFile(DSL), this.createCodePoints(DSL), this.createMetricsValues(DSL))
    );
  }
  ...
  private createCodePoints(DSL: string): CodePoint[] {
    ...
    switch (typeDSL) {
      case 'F':
        return [new CodePoint(CodePointType.File, ...args)];
      case 'C':
        result.push(new CodePoint(CodePointType.Class, ...args));
        break;
      case 'M':
        result.push(
        ...
        break;
      case 'f':
        result.push(new CodePoint(CodePointType.Method, ...args));
        break;
    }
