...
export class ExtractMethodMetricsAnalyzer implements MetricsAnalyzer {
  ...
  analyze(metricsList: Metrics[]): Metrics[] {
    return metricsList
      ...
      .filter(metrics => metrics.hasMetricsValue(...this.calculator.targetMetrics))
      .map(metrics => new Metrics(
        metrics.file,
        metrics.codePoints,
        this.calculator.calculate(this.calculator.targetMetrics
            .map(constructor => metrics.getMetricsByMetricsValue(constructor)!)
      )));
  }
}
