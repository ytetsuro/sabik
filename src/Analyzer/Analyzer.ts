...
export class Analyzer {
    ...
    private metricsAnalyze(metricsList: Metrics[]) {
        const analyzedMetricsList = this.metricsAnalyzers
            .flatMap(analyzer => analyzer.analyze(metricsList));

        this.setMetricsList(analyzedMetricsList);
    }

    private astAnalyze(fileAndMetricsList: {file: File, astNode: ASTNode}[]) {
        const metricsList = this.astAnalyzers
            .flatMap(analyzer => analyzer.analyze(fileAndMetricsList));
