...
describe('Analyzer class.', () => {
    describe('should return a list of metrics parsed from the file.', () => {
        it('', () => {
            ...
            const analyzer = new Analyzer(generator, [{
                analyze: (sourceList) => sourceList.map(source => new Metrics(
                    source.file,
                ...
                analyze: (metricsList) => metricsList.map(metrics => new Metrics(
                    metrics.file,
                    metrics.codePoints,
                    [{
            ...
            expect(actual[0].file).toStrictEqual({fullPath: '/dev/null', relativePath: 'null', extension: ''});
