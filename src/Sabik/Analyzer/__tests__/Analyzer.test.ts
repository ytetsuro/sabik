...
describe('Analyzer', () => {
    const analyzer = new Analyzer({
            complexityConverter: {
                convert: (_) => new ComplexityCountableNode({
                    DSL:'IN',
                    childs: [{DSL: 'I'}, {DSL: ''}]
                }),
            },
            halsteadConverter: {
                convert: (_) => new HalsteadCountableNode({
                    DSL: 'T',
                    text: '+',
                    childs: [
                        {DSL: 'N', text: '1'},
            ...
            lineOfCodeConverter: {
                convert: (_) => new LineOfCodeCountableNode(`1
                    2
                    // 3
                    4`)
            },
            astGenerator: {
                generate: () => new ASTNode(
                    ':root:0:0', {
    ...
    describe('.analyze()', () => {
        it('should returns file Analyzed object.', () => {
            const actual = analyzer.analyze(new File('/tmp/bar/foo.txt', 'bar/foo.txt'));
