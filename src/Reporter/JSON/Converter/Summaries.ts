...
export class Summaries {
  ...
  toJSON() {
    return [...this.summaries.entries()].reduce(
      (summaries, [summaryName, args]) => ({
        ...summaries,
  ...
  private convertSummary<T extends MetricsValue>(
    type: CalculateType,
    operation: CalculateOperator,
    metricsType: MetricsValueConstructor<T>
  ): DataModel {
    const calculator = type === 'file' ? this.fileCalculator : this.methodCalculator;
    const value: Metrics | number = (<(metricsValue: MetricsValueConstructor<T>) => Metrics | number>(
