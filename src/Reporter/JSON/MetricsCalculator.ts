...
export class MetricsCalculator {
    ...
    sum<T extends MetricsValue>(metricsValueConstructor: MetricsValueConstructor<T>): number
    {
       return this.metrics
        ...
        .reduce((sum, metrics) => sum.plus(new Decimal(Number(metrics))), new Decimal(0)).toNumber();
    }
    ...
    average<T extends MetricsValue>(metricsValueConstructor: MetricsValueConstructor<T>): number
    {
        return (new Decimal(this.sum(metricsValueConstructor)))
            .div(this.metrics.filter(metrics => metrics.hasMetricsValue(metricsValueConstructor)).length).toNumber();
    }

    max<T extends MetricsValue>(identity: MetricsValueConstructor<T>): Metrics {
        return this.metrics
            .filter(metrics => metrics.hasMetricsValue(identity))
            .reduce((max, metrics) => Number(max?.getMetricsByMetricsValue(identity) ?? -Infinity) > Number(metrics.getMetricsByMetricsValue(identity)) ? max : metrics) ?? null;
    }

    min<T extends MetricsValue>(identity: MetricsValueConstructor<T>): Metrics {
        return this.metrics
            .filter(metrics => metrics.hasMetricsValue(identity))
            .reduce((min, metrics) => Number(min?.getMetricsByMetricsValue(identity) ?? Infinity) < Number(metrics.getMetricsByMetricsValue(identity)) ? min : metrics) ?? null;
    }
}