...
describe('FileMetrics', () => {
    ...
    describe('.getMaximumComplexity()', () => {
        it('should returns maximumComplexity.', () => {
            ...
            const metrics = new FileMetrics('dummy.ts', {logical: 1, physical: 2}, [
                ...
                new Metrics('test2', [new MetricsValue(MetricsType.CongnativeComplexity, 3)], {start: 0, end: 1}),
            ]);
    ...
    describe('.getAverageComplexity()', () => {
        it('should returns average complexity.', () => {
            const metrics = new FileMetrics('dummy.ts', {logical: 1, physical: 2}, [
                new Metrics('test', [new MetricsValue(MetricsType.CongnativeComplexity, 4)], {start: 0, end: 1}),
                new Metrics('test2', [new MetricsValue(MetricsType.CongnativeComplexity, 3)], {start: 0, end: 1}),
            ]);

            expect(metrics.getAverageComplexity()).toStrictEqual(new MetricsValue(MetricsType.CongnativeComplexity, 3.5));
        });
    });
    ...
    describe('.getMaximumBugsDelivered()', () => {
        it('should returns maximumBugsDelivered.', () => {
            ...
            const metrics = new FileMetrics('dummy.ts', {logical: 1, physical: 2}, [
                new Metrics('test', [expected], {start: 0, end: 1}),
                new Metrics('test2', [new MetricsValue(MetricsType.BugsDelivered, 3)], {start: 0, end: 1}),
            ]);
    ...
    describe('.getSumBugsDelivered()', () => {
        it('should returns average bugsDelivered.', () => {
            const metrics = new FileMetrics('dummy.ts', {logical: 1, physical: 2}, [
                new Metrics('test', [new MetricsValue(MetricsType.BugsDelivered, 4)], {start: 0, end: 1}),
                new Metrics('test2', [new MetricsValue(MetricsType.BugsDelivered, 3)], {start: 0, end: 1}),
            ]);

            expect(metrics.getSumBugsDelivered()).toStrictEqual(new MetricsValue(MetricsType.BugsDelivered, 7));
        });
    });
