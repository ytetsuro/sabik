...
describe('Line of code Calculator Class.', () => {
  it('should count line by code', () => {
    ...
    const actual = analyzer.calculate(new LineOfCodeCountableNode(`
      // this is Test Code
      if (true) 
      {
          return 1;
    ...
    expect(Number(actual.find(row => row instanceof LogicalLineOfCode))).toBe(4);
    expect(Number(actual.find(row => row instanceof PhysicalLineOfCode))).toBe(6);
  });
});
