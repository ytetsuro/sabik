...
describe('Halstead complexity measures Class.', () => {
  describe('should getLength method return is operand and operator length.', () => {
    ...
    it('should return 3 when one vocabulary 2 operator, and one vocabulary 1 operand.', () => {
      const halstead = new Halstead(
        new OperandAndOperator(new Map([['dummy', 1]]), new Map([['dummy', 2]]))
      );
  ...
  describe('should cast JSON.', () => {
    it('should return all values.', () => {
      ...
      expect(JSON.stringify(halstead)).toBe(
        '{"operands":[["dummy1",1],["dummy2",1]],"operators":[["dummy",1]],"volume":4.754887502163468,"length":3,"vocabulary":3,"difficulty":0.25,"effort":1.188721875540867,"time":0.06604010419671484,"bugsDelivered":0.001584962500721156}'
      );
    });
  });
});
