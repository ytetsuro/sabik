...
export class Halstead {
  ...
  getVocabulary() {
    return (
      this.seed.getUniqueOperandCount() + this.seed.getUniqueOperatorCount()
    );
  }
  ...
  getDifficulty() {
    return (
      (this.seed.getUniqueOperatorCount() / 2) *
      (this.seed.getTotalOperatorCount() / this.seed.getUniqueOperandCount())
    );
  }
